...
Map the change to exactly one primary lesson. A related lesson can receive a cross-reference, but do not duplicate the principle. If a new principle is proposed, update `core-principles-baseline.md` and `principles.md` before all derived materials.
...
Update the applicable files among `README.md`, `principles.md`, `core-principles-baseline.md`, `facilitator-guide.md`, `slide-outline.md`, `live-demo-script.md`, and `system-map.md`. Keep the seven lessons aligned. Do not turn a 45-minute lesson into a file tour or a command tutorial.
...
Use the shared [workshop exercise](../../workshop-exercise.md) to connect lessons. Keep expected reasoning and a small evidence-based rubric available to facilitators. Teach portability through approved operation bindings and verified prerequisites, and keep model choices conditional on the selected role and target-platform support.
